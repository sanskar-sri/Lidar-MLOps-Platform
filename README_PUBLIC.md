...
- Airflow-triggered preprocessing pipeline for GPU worker execution
...
6. Poll Airflow for DAG status and task progress
...
| Data Explorer | Upload raw LiDAR data, browse registered datasets, and inspect analytics |
| Preprocessing | Configure and trigger Airflow preprocessing runs |
| Training | Monitor model training workflows |
| Postprocessing | Review downstream model outputs |
| Control Panel | Check compute nodes, service status, and runtime health |
